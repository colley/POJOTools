...
public class StringUtil {
    ...
    public static String add(String s, int count) {
        StringBuilder sb = new StringBuilder("");
    ...
    public static String trimTrailing(String s, char c) {
        return trimTrailing(s, new char[] {
                c
            });
    }
    ...
    public static int getDTDFilePath(String str, String packageName) {
        int    count = 0;
        String newpackagepath = packageName.trim();
        int    size = packageName.trim().length();
        int    newSize = newpackagepath.replaceAll("\\.", "").length();
        if ((size - newSize) > 0) {
            count += ((size - newSize) + 1);
        }

        String   workPath = System.getProperty("user.dir");
        String   newpath = str.substring(str.indexOf(workPath) + workPath.length() + 1, str.length());
        String[] args = newpath.split("\\\\");
        int      size1 = args.length;
        args = newpath.split("\\/");
        int size2 = args.length;
        count += ((size2 > size1) ? size2 : size1);
        return count;
    }

    public static void main(String[] args) {
        getDTDFilePath("E:\\github\\zj_java\\generatorPojo\\src/main/java", "com.test");
    }

    private static final Logger _log = Logger.getLogger(StringUtil.class.getName());
}
